...
DELAY_BETWEEN_TX = getattr(config, "DELAY_BETWEEN_TX", 90)
...
def swap_token(private_key):
    jumlah = acak_angka(1, 1.5)
    print(f"Jumlah Swap : {jumlah}")
    ...
    try:
        ...
        tx['gas'] = estimated_gas_swap + 60000 
    except Exception as e:
        tx['gas'] = 600000

    signed_tx = w3.eth.account.sign_transaction(tx, private_key)
    tx_hash = w3.eth.send_raw_transaction(signed_tx.rawTransaction)
    print(f"TX Hash: {EXPLORER}{tx_hash.hex()}")
...
def countdown(seconds):
    for i in range(seconds, 0, -1):
        print(f"Countdown untuk tx berikutnya: {i} detik", end='\r')
        time.sleep(1)
    print(" " * 50, end='\r')
...
# Process Add LP
        ...
        for lpi in range(1, jumlah_lp + 1):
            ...
            try:
                ...
                print(f"TX Hash: {EXPLORER}{tx_hash}")
                print("Add LP berhasil!")
                countdown(30)
...
if __name__ == "__main__":
    ...
    jumlah_swap = 30
    jumlah_lp = 10
    total_runs = 99999999  # total looping 
